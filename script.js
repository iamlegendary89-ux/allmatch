// script.js (updated with new questions from the document, adjusted vals and matching logic)

// User answers
let answers = {};
let currentQuestion = 0;

// Updated questions from ADVANCED_QUESTIONS with assigned vals (numerical for matching)
const questions = [
...
// 2025 phone data (updated categories based on new budget ranges: 1=<$300, 2=$300-500, 3=$500-800, 4=>$800; added softwareSupport 1-5 based on years, wireless/expandable/headphone yes/no)
const phones = [
...
// Submit and updated match (adjusted for new keys)
function submitQuiz() {
    const weightedKeys = ['battery', 'camera', 'gaming', 'softwareSupport']; // Updated for new priorities
    ...
    const matches = phones.map(phone => {
        ...
        // Primary use weight
        let primaryBonus = 0;
        if (answers.primaryUse === 1) primaryBonus = (phone.battery + phone.performance) / 10; // Social: battery + performance
        if (answers.primaryUse === 2) primaryBonus = phone.performance / 5; // Gaming: performance
        if (answers.primaryUse === 3) primaryBonus = phone.camera / 5; // Photo: camera
        if (answers.primaryUse === 4) primaryBonus = (phone.battery + phone.performance) / 10; // Work: battery + performance
        catScore += primaryBonus;

        let totalCat = catScore / 6; // Adjusted for added categories
        let score = (cos * 0.5 + totalCat * 0.5) * 100;
